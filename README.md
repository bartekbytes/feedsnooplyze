...
That’s why I built **FeedSnooplyze**:
...
Here’s a high-level look at how **FeedSnooplyze** works. (For a detailed breakdown, see the full documentation.)
...
## ⚙️ Configure FeedSnooplyze

FeedSnooplyze requires two configuration files:
...
- `feedsnooplyze.yaml.example` (Pages Configuration)

❗ For detailed setup instructions, see `docs/technical-overview.md`.

## 🏃‍♂️ Run FeedSnooplyze

You can run FeedSnooplyze in two ways:
...
- Run:
  - Run as a module (from the root folder) `python -m feedsnooplyze <your_arguments>` with the required arguments (see below) or
  - After installing application as a Python package (`pip install -e .`): `feedsnooplyze <your_arguments>` with the required arguments (see below)
